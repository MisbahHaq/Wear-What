    ...
    public class OrdersController : Controller
    {
        ...
        public async Task<IActionResult> RequestReturn(int orderItemId, string reason)
        {
            if (string.IsNullOrWhiteSpace(reason))
            {
                ...
                return RedirectToAction(nameof(Tracking), new { id = 0 });
            }
            ...
            if (orderItem.Order.Status != "Delivered")
            {
                ...
                return RedirectToAction(nameof(Tracking), new { id = orderItem.OrderId });
            }
        ...
        public async Task<IActionResult> ApproveReturn(int returnRequestId)
        {
            ...
            if (returnRequest == null || returnRequest.OrderItem == null)
                return NotFound();

            var currentUserId = GetCurrentUserId();
            if (returnRequest.SellerId != currentUserId && !User.IsInRole("Admin"))
                return Forbid();

            returnRequest.Status = "Approved";
            returnRequest.RefundStatus = "Refunded";
            returnRequest.SellerResponse = "Return approved";
            returnRequest.RespondedAt = DateTime.UtcNow;
            returnRequest.RefundedAt = DateTime.UtcNow;

            if (returnRequest.OrderItem.Product != null)
            {
                returnRequest.OrderItem.Product.StockQuantity += returnRequest.OrderItem.Quantity;
            }

            await _context.SaveChangesAsync();

            if (returnRequest.OrderItem.Order != null)
            {
                await AddStatusHistoryAsync(returnRequest.OrderItem.OrderId, "Returned", "Return approved and refunded");
            }

            if (!string.IsNullOrEmpty(returnRequest.BuyerId))
            {
                await SendNotificationAsync(returnRequest.BuyerId, $"Your return for order #{returnRequest.OrderItem.OrderId} was approved", $"/Orders/Tracking/{returnRequest.OrderItem.OrderId}");
            }

            TempData["StatusMessage"] = "Return approved.";
            return RedirectToAction(nameof(Dashboard));
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> RejectReturn(int returnRequestId)
        {
            var returnRequest = await _context.ReturnRequests
                .Include(r => r.OrderItem)
                .FirstOrDefaultAsync(r => r.Id == returnRequestId);
            ...
            returnRequest.SellerResponse = "Return rejected";
            returnRequest.RespondedAt = DateTime.UtcNow;
            ...
            if (returnRequest.OrderItem.Order != null)
            {
                await AddStatusHistoryAsync(returnRequest.OrderItem.OrderId, "ReturnRejected", "Return request rejected");
            }

            if (!string.IsNullOrEmpty(returnRequest.BuyerId))
            {
                await SendNotificationAsync(returnRequest.BuyerId, $"Your return for order #{returnRequest.OrderItem.OrderId} was rejected", $"/Orders/Tracking/{returnRequest.OrderItem.OrderId}");
            }
        ...
        public async Task<IActionResult> Analytics()
        {
            ...
            if (!shopIds.Any() && !User.IsInRole("Admin"))
            {
                return RedirectToAction("Index", "Profile");
            }
